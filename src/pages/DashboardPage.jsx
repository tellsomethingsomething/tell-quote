...
export default function DashboardPage({ onViewQuote, onNewQuote, onGoToOpportunities, onGoToKnowledge }) {
    ...
    const knowledgeStats = useMemo(() => {
        ...
        const typeLabels = {
            ...
            process_tip: 'Process Tips',
            regional_pattern: 'Regional Patterns',
        };
    ...
    return (
        <div className="h-[calc(100vh-60px)] overflow-y-auto">
            ...
            <div className="bg-dark-bg border-b border-dark-border p-3 sm:p-6">
                ...
                <div className="mt-6 card bg-gradient-to-br from-violet-900/20 to-violet-950/10 border-violet-800/20">
                    <div className="flex items-center justify-between mb-4">
                        <div className="flex items-center gap-2">
                            ...
                            <div>
                                <span className="text-sm font-semibold text-violet-400">AI Knowledge Base</span>
                                <p className="text-xs text-gray-500">Commercial intelligence for better quotes</p>
                            </div>
                        </div>
                        <button
                            onClick={() => onGoToKnowledge && onGoToKnowledge()}
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
                        <div className="text-center p-2 bg-dark-bg/50 rounded-lg">
                            <p className="text-[10px] text-gray-500 uppercase tracking-wider">Knowledge</p>
                            <p className="text-lg font-bold text-violet-400">{knowledgeStats.totalFragments}</p>
                            <p className="text-[9px] text-gray-600">{knowledgeStats.verifiedFragments} verified</p>
                        </div>
                        <div className="text-center p-2 bg-dark-bg/50 rounded-lg">
                            <p className="text-[10px] text-gray-500 uppercase tracking-wider">Learnings</p>
                            <p className="text-lg font-bold text-indigo-400">{knowledgeStats.totalLearnings}</p>
                            <p className="text-[9px] text-gray-600">{knowledgeStats.verifiedLearnings} applied</p>
                        </div>
                        <div className="text-center p-2 bg-dark-bg/50 rounded-lg">
                            <p className="text-[10px] text-gray-500 uppercase tracking-wider">Confidence</p>
                            <p className="text-lg font-bold text-emerald-400">
                                {knowledgeStats.avgConfidence > 0 ? `${(knowledgeStats.avgConfidence * 100).toFixed(0)}%` : '-'}
                            </p>
                            <p className="text-[9px] text-gray-600">avg score</p>
                        </div>
                        <div className="text-center p-2 bg-dark-bg/50 rounded-lg">
                            <p className="text-[10px] text-gray-500 uppercase tracking-wider">Pending</p>
                            <p className={`text-lg font-bold ${knowledgeStats.pendingCount > 0 ? 'text-amber-400' : 'text-gray-500'}`}>
                                {knowledgeStats.pendingCount}
                            </p>
                            <p className="text-[9px] text-gray-600">to review</p>
                        </div>
                    </div>

                    {/* Knowledge by Type */}
                    {knowledgeStats.topTypes.length > 0 && (
                        <div className="flex flex-wrap gap-2 mb-3">
                            {knowledgeStats.topTypes.map(({ type, label, count }) => (
                                <div key={type} className="flex items-center gap-2 px-2 py-1 bg-dark-bg/30 rounded text-xs">
                    ...
                    {knowledgeStats.totalFragments === 0 && (
                        <div className="border-t border-violet-800/20 pt-3 text-center">
                            <p className="text-xs text-gray-500 mb-2">
                                Start building your commercial intelligence by capturing market insights, pricing strategies, and client preferences.
                            </p>
                            <button
                                onClick={() => onGoToKnowledge && onGoToKnowledge()}
                                className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-violet-400 hover:text-white bg-violet-500/10 hover:bg-violet-500/20 rounded-lg transition-colors"
                            >
                    ...
                    {knowledgeStats.pendingCount > 0 && (
                        <div className="border-t border-violet-800/20 pt-3">
                            <button
                                onClick={() => onGoToKnowledge && onGoToKnowledge()}
                                className="w-full flex items-center justify-between p-2 rounded-lg bg-amber-500/10 hover:bg-amber-500/15 transition-colors group"
                            >
                                <div className="flex items-center gap-2">
                                    <svg className="w-4 h-4 text-amber-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                                    ...
                                    <span className="text-xs text-amber-400">
                                        {knowledgeStats.pendingFragments > 0 && `${knowledgeStats.pendingFragments} knowledge items`}
                                        {knowledgeStats.pendingFragments > 0 && knowledgeStats.pendingLearnings > 0 && ' and '}
                                        {knowledgeStats.pendingLearnings > 0 && `${knowledgeStats.pendingLearnings} learnings`}
                                        {' '}awaiting review
                                    </span>
                                </div>
                                <svg className="w-4 h-4 text-gray-600 group-hover:text-amber-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
                                </svg>
                            </button>
                        </div>
