...
const tabs = [
    {
        ...
        icon: FileText,
        description: 'Create detailed, beautiful quotes in minutes. Drag-and-drop line items, apply global markups, and win more work.',
    ...
    },
    {
        ...
        icon: Clapperboard,
        description: 'Visualize your entire production pipeline. Track crew, equipment, and shoot dates on a unified Kanban board.',
    ...
    },
    {
        ...
        icon: DollarSign,
        description: 'Real-time visibility into project profitability. Track revenue, expenses, and agency margins automatically.',
    ...
    },
    {
        ...
        icon: Users,
        description: 'The command center for your agency. See active projects, pending quotes, and team capacity at a glance.',
                ...
                <div className="lg:col-span-4 flex flex-col gap-4">
                    {tabs.map((tab) => (
                        <button
                            ...
                            className={`text-left p-6 rounded-xl border transition-all duration-300 group ${activeTab.id === tab.id
                        ...
                        >
                            <div className="flex items-center gap-4 mb-2">
                                <div className={`p-2 rounded-lg ${activeTab.id === tab.id ? tab.bg : 'bg-marketing-surface border border-marketing-border'}`}>
                                    <tab.icon className={`w-6 h-6 ${activeTab.id === tab.id ? tab.color : 'text-marketing-text-secondary'}`} />
                                </div>
                                <h3 className={`font-bold text-lg ${activeTab.id === tab.id ? 'text-white' : 'text-marketing-text-secondary group-hover:text-white'}`}>
                                    {tab.label}
                                </h3>
                            </div>
                            <p className={`text-sm pl-[3.5rem] leading-relaxed transition-colors ${activeTab.id === tab.id ? 'text-marketing-text-secondary' : 'text-marketing-text-secondary/50'
                                }`}>
                                {tab.description}
                            </p>
                        </button>
                        ...
                        <AnimatePresence mode='wait'>
                            ...
                            >
                                <MotionScreenGrab
                                    src={activeTab.image}
                                    alt={activeTab.label}
                                    className="shadow-2xl border border-marketing-border/50 rounded-xl"
                                />
                            </motion.div>
                        </AnimatePresence>
                    </div>
